...
def sankey_departments(df, col_old, col_new, title="ซ่อนชื่อ (ระดับฝ่าย) – NHSO Theme"):
    ...
    # Sort ตาม total แต่ละฝั่ง เพื่อจัดลำดับโหนดให้อ่านง่าย
    ...
    # สีโหนดและสีลิงก์ (อิงสีโหนดฝั่งซ้าย)
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            ...
            value=values,
            color=link_colors,
            hovertemplate=(f"{col_old}: %{ '{' }source.label}<br>"
                           f"{col_new}: %{ '{' }target.label}<br>"
...
def sankey_with_names(df, col_old, col_new, col_name, title="แสดงชื่อบุคคล – NHSO Theme"):
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            ...
            color=link_colors,
            hovertemplate=(f"{col_old}: %{ '{' }source.label}<br>"
                           f"{col_new}: %{ '{' }target.label}<extra></extra>")
        ),
    )])
...
# เลือกคอลัมน์แบบยืดหยุ่น (รองรับชื่อคอลัมน์ต่างกันเล็กน้อย)
...
if len(filt_df) == 0:
    ...
    # ดาวน์โหลด HTML แบบ interactive
